...
evaluateRouter.post(
  ...
  async (req: AuthenticatedRequest, res, next) => {
    ...
    if (evalError) {
      throw createError(`Failed to save evaluation: ${evalError.message}`, 500);
    }
    ...
    try {
      ...
      try {
        ...
        if (aiError.status === 401 || aiError.message?.includes('401')) {
          errorMessage = 'NVIDIA API authentication failed. Please check your NVIDIA_API_KEY in the environment variables. The API key may be invalid, expired, or missing.';
      ...
      // evaluationResult is already parsed from evaluateCodeComplete
      ...
      // Send completion
      const evaluationId = savedEvaluation?.id || (Array.isArray(savedEvaluation) ? savedEvaluation[0]?.id : null);
      res.write(`data: ${JSON.stringify({ 
        type: 'complete', 
        done: true,
      ...
      const errorMessage = streamError.message || 'An error occurred during evaluation';
      res.write(`data: ${JSON.stringify({ 
        type: 'error',
        error: errorMessage,
        progress: 0
